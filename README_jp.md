...
> [!NOTE]
> **`02_vpnSetup.sh`について**
> - Pritunl VMはVPNで外部から各テナント内のVMにアクセスするための機能です。(tailscale/踏み台など)異なる方法でアクセス経路を作りたい場合などは、本スクリプトの実行をスキップ可能です。その際は各テナント(vnetpjXX)へ到達する経路を別途用意し、テナント境界のセキュリティ確保もあわせて適切に実施してください。
