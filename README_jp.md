...
```
> **補足:** `02_vpnSetup.sh` では、VPN 用 UDP ポートの到達性確認のため、MSL Setup の公開プローブ API に対して HTTPS リクエストを送信します。  
> このプローブは、そのリクエストを送った元のグローバル IP アドレスに対してのみ UDP パケットを返す方式で動作し、任意の第三者 IP アドレスを指定して送信する仕組みではありません。  
> この公開プローブ API は日本国内のホスト上で運用しています。
> この確認は、ルータのポートフォワード設定や回線仕様により VPN サーバへ外部到達できない状態を、インストール時点で早期検出するためのものです。
