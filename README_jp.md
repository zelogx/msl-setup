...
# Phase 2: (任意) VPN セットアップ (Pritunl VM 展開 + 設定)
./02_vpnSetup.sh jp   # 言語: en|jp (デフォルトen)
```

> [!NOTE]
> **`02_vpnSetup.sh`について**
> - Pritunl VMはVPNで外部から各テナント内のVMにアクセスするための機能です。(tailscaleなど)異なる方法でアクセス経路を作りたい場合などは、本スクリプトの実行をスキップ可能です。その場合、テナント、MainLanのセキュリティの確保は、別途適切に実施する必要があります。
> - インストール完了時点では、Pritunl VM は自動的に HA リソースとして登録されません。クラスタ運用で Pritunl VM の HA 管理が必要な場合は、別途 Proxmox HA の設定を行ってください。
> - VPN 用 UDP ポートの到達性確認のため、MSL Setup の公開プローブ API に対して HTTPS リクエストを送信します。ルータのポートフォワード設定や回線仕様により VPN サーバへ外部到達できない状態を、インストール時点で早期検出するためのものです。
> - 送信されるデータはプローブ先のUDPポート番号のみです。送信元のパブリックIPアドレスとそのUDPポート番号のみログとして保持されます。
> - プローブリクエスト送信元のグローバル IP アドレスに対してのみ UDP パケットを返す方式で動作し、任意の第三者 IP アドレスを指定して送信する仕組みではありません。  
> - この公開プローブ API は日本国内のホスト上で運用しています。


```bash
# Phase 3 (Corporate エディション専用): RBAC Self-Care ポータルセットアップ
...
> [!NOTE]
> **msldhcpについての補足:**
> - 対象は「VXLAN zoneかつvnetpj*」のVNetのみです。DHCP Rangeは`0102_setupNetwork.sh`実行時に自動設定されるため、通常は追加設定なしでそのまま使えます。
...
#### (f) **各PJ(vnetpjxx)に割り当てるネットワークアドレス(新設):** (例：`172.16.16.0/20`)
   ...
   - このVMへのFW設定はDatacenterレベルのルールで制御される（同一プロジェクトのネットワーク内の通信は許可、他のプライベートネットワーク宛の通信は遮断）。
   - Pritunlのorgに対応
...
- **コメントが `MSLSetup` で始まるファイアウォールルールは MSL Setup が管理します**  
   MSL Setup は、自身が作成した Datacenter のファイアウォールルールをコメントで識別し、再実行・リストア・アンインストール時に削除します。  
   これらのルールのコメントは変更しないでください。また、ご自身で追加するルールには `MSLSetup` で始まるコメントを使用しないでください。コメントを変更したルールは MSL Setup では削除されず、一致するコメントを付けた独自ルールは削除される場合があります。

- **MSL Setup の導入後に作成した SDN オブジェクトと設定は、再実行・リストア・アンインストールで元に戻ります**  
   MSL Setup は初回実行時に、導入前の SDN とファイアウォールの状態をバックアップします。`01_networkSetup.sh` の再実行、`01_networkSetup.sh --restore`、`99_uninstall.sh` はいずれも、最初にその状態へ戻します。バックアップに無い SDN の Zone・VNet・Subnet・IPSet は削除され、Datacenter / ホストのファイアウォールのオプションはバックアップ時の値に戻ります。  
   MSL Setup の導入中にご自身で作成した Zone・VNet・IPSet も対象になります。MSL Setup の導入前に作成するか、いったんアンインストールしてから作成し、再度導入してください。バックアップはリストア・アンインストールの成功後に削除され、次回の実行時に取り直されます。
...
## 7. トラブルシューティング: UDP port forwarding validation failed

このチェックでは、MSL Setup の公開プローブ API に対して HTTPS リクエストを送信し、そのリクエストを送った元のグローバル IP アドレスに対してのみ UDP パケットを返すことで、指定した VPN 用 UDP ポートの到達性を確認します。

つまり、インターネット上の第三者ホストへ任意に送信する仕組みではなく、
「HTTPS リクエストを送信した元のグローバル IP アドレスに対して、指定ポートへ到達できるか」を確認するための限定的な疎通確認です。
